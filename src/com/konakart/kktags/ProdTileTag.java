...
public class ProdTileTag extends BaseTag {
	...
	// Required since we may show more than one tile of the same product on a
	// page which would make
	// the ids match.
	...
	public int doStartTag() throws JspException {
		try {
			setEng((com.konakart.al.KKAppEng) pageContext.getSession()
					.getAttribute("konakartKey"));
	...
	/*	com.konakart.al.KKAppEng kkEng = (com.konakart.al.KKAppEng) pageContext
					.getSession().getAttribute("konakartKey");
			com.konakart.al.CustomerMgr customerMgr = kkEng.getCustomerMgr();
			int j = 0;
			com.konakart.appif.BasketIf basketItems[] = customerMgr
					.getCurrentCustomer().getBasketItems();
			for (com.konakart.appif.BasketIf item : basketItems) {
				if (item.getProduct() != null
						&& item.getProductId() == prod.getId()) {
					j = 1;
					break;
				}
			}
		...
		/*	com.konakart.al.KKAppEng kkEng = (com.konakart.al.KKAppEng) pageContext
			...
			// Float over
			sb.append(getStartDiv("item-over",
					rand + "ov-" + Integer.toString(prod.getId())));
			...
			if (eng.getQuotaMgr().canAddToBasket(prod, null) > 0) {
				...
				sb.append(getStartA(
						"add-to-cart-button button small-rounded-corners", "#",
			...
			if (eng.isWishListEnabled()) {
				...
				sb.append(getStartA("add-to-wishlist", "#", rand + "atc-"
						+ Integer.toString(prod.getId())));
			...
			if (prod.getPromotionResults() != null
					&& prod.getPromotionResults().length > 0) {
	...
	private void getImageLink(StringBuffer sb, int size) {
		String url;
		if (eng.isPortlet()) {
			url = createPortalURL("SelectProd.action",
					new NameValue[] { new NameValue("prodId", prod.getId()) });
		} else {
	...
	private BigDecimal getPrices(StringBuffer sb) throws KKAppException {
		...
		if (eng.displayPriceWithTax()) {
			if (prod.getSpecialPriceIncTax() != null) {
				saving = prod.getPriceIncTax().subtract(
						prod.getSpecialPriceIncTax());
			...
			if (prod.getSpecialPriceExTax() != null) {
				saving = prod.getPriceExTax().subtract(
						prod.getSpecialPriceExTax());
				sb.append(getStartDiv("price old"));
		...
		for (int i = 0; i < customerMgr.getCurrentCustomer().getBasketItems().length; i++) {
			com.konakart.appif.BasketIf item = customerMgr.getCurrentCustomer()
					.getBasketItems()[i];
			if (item.getProduct() != null
					&& item.getProduct().getId() == prod.getId()) {
				j = 1;
				break;
			}
		}
