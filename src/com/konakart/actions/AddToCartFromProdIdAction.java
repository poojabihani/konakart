...
public class AddToCartFromProdIdAction extends AddToCartOrWishListBaseAction {
	...
	public String execute() {
		...
		try {
			...
			if (log.isDebugEnabled()) {
				log.debug("Product Id of selected product from application = "
						+ getProdId());
			}

			// Get the product from its Id
			kkAppEng.getProductMgr().fetchSelectedProduct(getProdId());
			ProductIf selectedProd = kkAppEng.getProductMgr()
					.getSelectedProduct();
			...
			/*
			 * If the product has options then we can't add it to the cart
			 * directly. We must first go to the product details page so that
			 * the options can be selected.
			 */
			if (selectedProd.getOpts() != null
					&& selectedProd.getOpts().length > 0) {
				/*
				 * Set just the product id. In the Javascript we work out the
				 * redirect URL so that it works also when we are running as a
				 * portlet
				 */
			...
			this.addToCart(kkAppEng, selectedProd, null, Integer.parseInt(getProdQty()));
